...
def validate_entry(entry: dict, path: Path, line_no: int) -> list[str]:
    errors: list[str] = []
...
def validate_archive(archive_root: Path) -> dict:
    ...
    for path in files:
        errors.extend(validate_path_shape(path, archive_root))
...
def main() -> int:
    args = parse_args()
    report = validate_archive(Path(args.archive_root).expanduser())
    if args.json:
